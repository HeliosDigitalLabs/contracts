...
use cw721::{
    ...
    OwnerOfResponse, TokensResponse, TokensFullResponse, TokenFullInfo,
};
    ...
    Q: CustomMsg,
{
    pub fn query(&self, deps: Deps, env: Env, msg: QueryMsg<Q>) -> StdResult<Binary> {
        match msg {
            ...
            QueryMsg::TokensInfo {
                ...
                start_after,
                limit,
            } => to_json_binary(&self.tokens_info(deps, owner, start_after, limit)?),
            QueryMsg::CheckOwnership { owner, base_ids } => {
                to_json_binary(&self.check_ownership(deps, owner, base_ids)?)
            }
        }
    }
    ...
    fn tokens_info(
        ...
        start_after: Option<String>,
        limit: Option<u32>,
    ) -> StdResult<TokensFullResponse> {
        let limit = limit.unwrap_or(DEFAULT_LIMIT).min(MAX_LIMIT) as usize;
        let start = start_after.map(|s| Bound::ExclusiveRaw(s.into()));
    
        let owner_addr = deps.api.addr_validate(&owner)?;
        let token_infos: Vec<TokenFullInfo> = self
            .tokens
            .idx
            .owner
            .prefix(owner_addr)
        ...
        Ok(TokensFullResponse { tokens: token_infos })
    }
    ...
    fn check_ownership(
        ...
        let results = base_ids.into_iter().map(|base_id| {
            let tokens = self.tokens.idx.owner.prefix(owner_addr.clone());
            let mut range = tokens.range(deps.storage, None, None, Order::Ascending);

            range.any(|item| {
                match item {
                    Ok((token_id, _)) => token_id.starts_with(&base_id),
                    Err(_) => false,
                }
            })
        }).collect::<Vec<_>>();
